---
id: d1e5a760-453c-4f89-81d1-605f0d4a4ea7
blueprint: page
title: 'Artist Opportunities'
author: 916c3785-d212-40bc-89ff-572097ac9582
updated_by: 9692c24d-4d69-4be7-a897-ad4edab416de
updated_at: 1776352063
turn_off_hero: false
inside_hero_content:
  -
    type: paragraph
    content:
      -
        type: text
        text: 'GCCA supports working visual artists in many ways, including providing studio spaces for rent at affordable rates. Artists working in visual art mediums such as drawing/illustration, digital art, encaustics, fiber arts/textiles, jewelry, mixed media/collage, painting, photography, and printmaking may apply for a studio. We currently do not have studios for ceramics, woodworking, glass, or metalworking.'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Although a minimum number of hours is not required, GCCA studios are intended for working artists so we request that artists spend time working in their studios a few days each week, as well as participate in Nightshifts and fundraising events. Patrons visiting GCCA are eager to see works-in-progress and are interested in purchasing art!'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Private, locking studios, as well as open, university-style studios are available, and cost is based on square footage.'
inside_hero_image: studio-space_6_21_.jpg
blocks:
  -
    id: miy003et
    panels:
      -
        id: miy006js
        panel_content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'If you’re craving a quiet, inspiring place to get lost in your work but aren’t ready to commit to a full studio rental, our Creative Space Access Hours at GCCA are designed to give you exactly that. These sessions offer an open, unstructured studio environment with no instruction and no formal programming, simply dedicated time and room to create at your own pace. '
        open_in_new_tab: false
        panel_image:
          - creative-access-hours-(1).jpg
        type: panel_group
        enabled: true
        panel_title_size: large
      -
        id: miy00vyl
        panel_content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'Artists can drop in for $25 per hour as needed, or choose our new $200 monthly option for unlimited studio access throughout the month. Please note that spaces may be shared with other working artists, adding to the sense of community and creative energy within the building. '
              -
                type: hardBreak
              -
                type: hardBreak
              -
                type: text
                text: 'Whether you’re polishing a new series, experimenting with materials, or just seeking a refuge from home distractions, we’re here to make space for your process. To reserve your spot or learn more, reach out to our Operations Director, Alicia Mizuno, at '
              -
                type: text
                marks:
                  -
                    type: link
                    attrs:
                      href: 'mailto:alicia@artcentergreenville.org'
                      rel: null
                      target: null
                      title: null
                text: alicia@artcentergreenville.org
              -
                type: text
                text: .
        open_in_new_tab: false
        panel_image:
          - creative-access-hours-(1)-copy-2.jpg
        type: panel_group
        enabled: true
        panel_title_size: large
      -
        id: miy01np6
        panel_content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'Available Spaces:'
          -
            type: bulletList
            content:
              -
                type: listItem
                content:
                  -
                    type: paragraph
                    content:
                      -
                        type: text
                        text: 'Gray Loft'
              -
                type: listItem
                content:
                  -
                    type: paragraph
                    content:
                      -
                        type: text
                        text: 'Main Gallery Classroom'
              -
                type: listItem
                content:
                  -
                    type: paragraph
                    content:
                      -
                        type: text
                        text: 'Conference Room'
              -
                type: listItem
                content:
                  -
                    type: paragraph
                    content:
                      -
                        type: text
                        text: 'Jewelry Classroom*'
          -
            type: paragraph
          -
            type: paragraph
            content:
              -
                type: text
                text: '*Separate booking rates. Requires safety training before first use.'
        open_in_new_tab: false
        panel_image:
          - creative-access-hours-(1)-copy-3.jpg
        type: panel_group
        enabled: true
        panel_title_size: large
    type: sticky_panels
    enabled: true
  -
    id: maxvubk1
    images:
      - studio-rental_1.jpg
      - faith-hudgens-operations-studio.jpg
      - gcca-arp-02007.jpg
      - studio-space_6_21_.jpg
      - gcca-showcase_0087.jpg
      - gcca-showcase_0093.jpg
      - kephira-in-studio.jpg
      - gcca-showcase_0058.jpg
    type: full_width_carousel
    enabled: true
  -
    id: may8ic9h
    section_heading: 'Studio Rental'
    text:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'GCCA supports working visual artists in many ways, including providing studio spaces for rent at affordable rates. Artists working in visual art mediums such as drawing/illustration, digital art, encaustics, fiber arts/textiles, jewelry, mixed media/collage, painting, photography, and printmaking may apply for a studio. We currently do not have studios for ceramics, woodworking, glass, or metalworking.'
      -
        type: paragraph
        content:
          -
            type: text
            text: 'Although a minimum number of hours is not required, GCCA studios are intended for working artists so we request that artists spend time working in their studios a few days each week, as well as participate in First Fridays and fundraising events. Patrons visiting GCCA are eager to see works-in-progress and are interested in purchasing art!'
      -
        type: paragraph
        content:
          -
            type: text
            text: 'Private, locking studios, as well as open, university-style studios are available, and cost is based on square footage.'
    form_embed: '<div style="width:100%;height:500px;" data-fillout-id="bL9u8efP4wus" data-fillout-embed-type="standard" data-fillout-inherit-parameters data-fillout-dynamic-resize></div><script src="https://server.fillout.com/embed/v1/"></script>'
    type: form_and_text
    enabled: true
open_in_new_tab: false
---
